...
with st.form("resume_builder"):
    panel(
        "Resume content",
        "Complete the required fields and preview the document before downloading.",
    )
    ...
    with proof_tab:
        ...
        st.caption(
            "Use one line per certification, project bullet, or experience bullet for the cleanest PDF formatting."
        )
...
with preview_tab:
    with st.container(border=True):
        panel(
            "Resume preview",
            "This preview mirrors the content that will be sent to the PDF generator.",
        )
        ...
        st.caption(
            " | ".join(part for part in contact_parts if part) or "Contact details will appear here"
        )
        sections = [
        ...
        if errors:
            ...
            with st.spinner("Generating professional PDF..."):
                try:
                    ...
                    st.error(
                        "Resume PDF could not be generated. Please review the content and try again."
                    )
                else:
                    try:
                        add_activity(
                            "Resume PDF generated",
                            f"Generated resume for {resume_data['name'] or 'learner'}",
                        )
